...
# API to check if all scenarios have been played
@game_bp.route("/check_scenarios", methods=["GET"])
def check_scenarios():
    game_done = False
    if game_state["scenarios_completed"] >= len(scenario_files):
        game_done = True
        return jsonify({
            "message": "All scenarios completed",
            "game_finished": game_done
        })
    else:
        return jsonify({
            "game_finished": game_done
...
def next_scenario():
    ...
    game_state["current_scenario_index"] += 1
...
# API to get players stats once the game is finished
@game_bp.route("/finish", methods=["GET"])
def finish_game():
    return jsonify({
        "total_lives_lost": game_state["total_lives_lost"],
        "wrong_choices_made": game_state["wrong_choices_made"],
        "scenarios_completed": game_state["scenarios_completed"],
        "failures": game_state["failures"]
    })
...
def get_choice_outcome(dilemma_id, choice_id):
    ...
    outcome_type = choice.get("type")
    if outcome_type == "wrong":
        game_state["total_lives_lost"] += 1
        game_state["wrong_choices_made"] += 1
    elif outcome_type == "lose":
        game_state["total_lives_lost"] += 3
        game_state["wrong_choices_made"] += 1 
